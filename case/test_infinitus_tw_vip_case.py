...
class Test_Main_Case_TW_VIP(unittest.TestCase, PageCommon):
    ...
    #@unittest.skip("暂时跳过")
    # tw vip邮箱用户登录
    @BeautifulReport.add_test_img("test_2_vip_loginemail_tw")
    def test_2_vip_loginemail_tw(self):
        # log 信息
        ...
        time.sleep(5)
        result = self.iselementexisted(*InfinitusLocator.rebate_xpath)
        #self.save_img("test_2_vip_loginemail_tw")
        print(result)
        self.assertEqual(result, True, "登录成功后显示回赠金按钮")
    ...
    #@unittest.skip("暂时跳过")
    def test_3_searchandadd_tw(self):
        # log 信息
